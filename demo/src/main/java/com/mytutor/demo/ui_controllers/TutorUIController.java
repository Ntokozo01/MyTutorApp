...
public class TutorUIController {
    ...
    /**
     * Gets the SessionExtra object of an activity session and checks which session and activity the tutor has signed up for
     * @param sessionID: activity session ID for the tutor signups
     * @return session extra object with set attribute for the course activity session
     * @throws SQLException
     */
   ...
   /**
    * Gets the list of Event objects to be shown on the calendar for the tutor sign-ups to course activity sessions
    ...
    public String addGetSignup(@PathVariable("sessionID") int sessionID, HttpSession httpSession) {
        ...
        try {
            ...
            httpSession.setAttribute("resultmessage", errorMessage);
        }
        return "redirect:/tutor/homepage";
    }
